...
#Implement the focal loss fucntion
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(files_dir):
        if filename.endswith(".csv"):
            ...
            with open(file_path, 'r') as file:
                ...
                vectors.extend([row for row in csv_reader])
                if len(vectors) < max_length:
                    padded_vectors = vectors + [[0.0] * len(vectors[0])] * (max_length - len(vectors))
                else:    
    ...
    alpha_vec = [ 0.21743397315629961, 11.679069767441861,7.533,10.248979591836735,11.7703125] 
    for a in alpha_vec:
        print(a)


    model = load_model('Bi-LSTM_5_Classes.h5', custom_objects={'focal_loss_fixed': focal_loss(alpha_vec),'AttentionLayer': AttentionLayer})
